...
def conv_block(filters, stage, block, strides=(2, 2), leaky_relu=False, **kwargs):
    ...
    def layer(input_tensor):
        ...
        if leaky_relu:
            x = layers.LeakyReLU(name=relu_name + '2')(x)
        else:
...
def identity_block(filters, stage, block, leaky_relu=False, **kwargs):
    ...
    for stage, rep in enumerate(model_params.repetitions):
        for block in range(rep):
            ...
            if stage == 0 and block == 0:
                x = conv_block(filters, stage, block, strides=(1, 1), leaky_relu=leaky_relu, **kwargs)(x)

            elif block == 0:
                x = conv_block(filters, stage, block, strides=(2, 2), leaky_relu=leaky_relu, **kwargs)(x)

            else:
                x = identity_block(filters, stage, block, leaky_relu=leaky_relu, **kwargs)(x)
...
def ResNeXt50(input_shape=None, input_tensor=None, weights=None, classes=1000,
...
def ResNeXt101(input_shape=None, input_tensor=None, weights=None, classes=1000, leaky_relu=False,
